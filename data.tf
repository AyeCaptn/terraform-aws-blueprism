...
data "template_file" "blueprism_appserver_setup" {
  ...
  vars = {
    ...
    create_custom_users = length(var.appserver_windows_custom_user_username) > 0 && length(var.appserver_windows_custom_user_password) > 0 ? join(
      "; ",
      data.template_file.appserver_create_custom_users.*.rendered,
    ) : ""
...
data "template_file" "blueprism_client_setup" {
  ...
  vars = {
    ...
    create_custom_users = length(var.client_windows_custom_user_username) > 0 && length(var.client_windows_custom_user_password) > 0 ? join(
      "; ",
      data.template_file.client_create_custom_users.*.rendered,
    ) : ""
    blueprism_installer_path   = var.blueprism_installer_path
...
data "template_file" "blueprism_resource_setup" {
  ...
  vars = {
    ...
    create_custom_users = length(var.resource_windows_custom_user_username) > 0 && length(var.resource_windows_custom_user_password) > 0 ? join(
      "; ",
      data.template_file.resource_create_custom_users.*.rendered,
    ) : ""
    blueprism_installer_path   = var.blueprism_installer_path
